  ...
  void operator()(ADvector& fg, const ADvector& vars) {
    ...
    // the Solver function below.
        ...
        for (auto t = 0u; t < N; t++) {
            ...
            fg[0] += c_ref_state[2] * CppAD::pow(vars[v_start + t] - MPC::ref_v, 2);
        }
...
double MPC::mph_to_mps_factor = 0.44704;
double MPC::ref_v = 40.;
double MPC::latency = 0.1;
...
std::pair<Eigen::VectorXd, Eigen::VectorXd> MPC::Transform2VehicleCoord(
...
{
  x_    = x0_ + v0_ * cos(psi0_) * latency;
  y_    = y0_ + v0_ * sin(psi0_) * latency;
  psi_  = v0_ / Lf * delta * latency;
  cte_  = cte0_ + v0_ * sin(epsi0_) * latency;
  epsi_ = epsi0_ + v0_ / Lf * delta * latency;
  v_    = v0_ + a_ * latency;
}
